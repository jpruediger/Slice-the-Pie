import React, { useRef, useEffect, useState } from 'react';
import EntryBox from "./entryBox.jsx"

// compile piechart and entryboxes for modular use, just add ID prop and delete IDs not in use
// should not be too bad
function PieChartExpenditure (props) {


  const selector="#"+props.id


  const idattr=props.id


  // setup callback support
  const callback = props.callback;
  
    // gets connected to svg drawn by React by variable name and its ref attribute
    let d3Container = React.createRef(null);
    
    let dimensions = ({
      width: props.width ? props.width : 200,
      height: props.height ? props.height : 200,
      padding: props.padding ? props.padding : 50
    });

    let [data, setData] = useState([...props.data])
    
    let pie = d3.pie()
                .sort(null)
                .value(d => d.value);

    let arc = d3.arc()
                .innerRadius(0)
                .outerRadius(Math.min(dimensions.width, dimensions.height) / 2 - 1);
  
    


  // init code - only run on startup
  useEffect(function() {

      if (!(d3Container.current && data)) 
        return;
      
    // TODO: PROBLEM WITH SELECTING PREVIOUS DRAWN PIECHART...
    // ASSIGN UNIQUE ID PER
    let chartSvg = d3.select(d3Container.current);
    
    // create title
    chartSvg.append("text")
      .attr("x", (dimensions.width)/2 + dimensions.padding)             
      .attr("y", 0 + dimensions.padding/2)
      .attr("text-anchor", "middle")
      .style('fill', 'white')
      .style('font-weight', '500')
      .style("font-size", "15px")
      .text(props.title);


    chartSvg.append('g')
      
      .attr('id', idattr)
      .attr('class', 'chart-content')
      .attr('transform', `translate(
      ${dimensions.width / 2 + dimensions.padding},
      ${dimensions.height/2 + dimensions.padding})`)
      .attr("overflow", "visible");



    },[]);

  // update function
  useEffect(function() {


    
    if (!(d3Container.current && data)) 
        return;


    // filter out empty portion, do not want
    let new_data = []
    for (let i = 0; i < data.length; i++) {
      if (data[i].name !== '$empty') {
        new_data.push(data[i])
      }
    }    

    // perfrom data swap
    data = new_data
    
    const valueSum = d3.sum(data, d => d.value);
    if (100 - valueSum > 0) {
      data.push({
        name: '$empty',
        value: (100 - valueSum),
        color: '#7f8187'
      });
    }

    const arcs = pie(data);

    const g = d3.select(selector);
    g.selectAll('path')
        .data(arcs)
        .join('path')
        .on('mouseover', (event, d) => onSliceOver(event, d))
        .on('mouseout', (event, d) => onSliceOut(event, d))
        .transition().duration(500)
        .attr('fill', d => d.data.color)
        .attr('transform', d => d.data.name == '$empty' ? 'scale(0.95)' : '')
        .attr('d', arc);
    }, [props.data, d3Container.current ]
  ); // useEffect update function

  
  function onSliceOver(event, d) {
    if (d.data.name == '$empty')
      return;

    let pointer = d3.pointer(event);

    d3.select(event.currentTarget)
      .transition().duration(200)
      .attr('transform', 'scale(1.1)')
// pointer[0]
// pointer[1] - 20
//'fill', getColorValue(d.data.color) > 150 ? '#000000' : '#ffffff'
    const g = d3.select(selector);
    g.select('#chart-tooltip').remove();
    g.append('text')
        .attr('x', pointer[0])
        .attr('y', pointer[1] - 20)
        .attr('fill', 'fill', getColorValue(d.data.color) > 150 ? '#000000' : '#ffffff')
        
        .attr('text-anchor', 'middle')
        .attr('id', 'chart-tooltip')
        .style('fill', 'white')
        .style('text-shadow', '-1px -1px 0 #000, 1px -1px 0 #000, -1px 1px 0 #000, 1px 1px 0 #000')
        
        .text(`${d.data.name} ${d.data.value.toFixed(2)}%`);
  }

  function onSliceOut(event, d) {
    if (d.data.name == '$empty')
      return;

    d3.select(event.currentTarget)
      .transition().duration(200)
      .attr('transform', 'scale(1)')

     const g = d3.select(selector);
     g.select('#chart-tooltip').remove();
  }

  function getColorValue(color) {
    let sumVal = 0;
    for (let i = 1; i < color.length; i+=2) {
      let val = parseInt(`0x${color[i]}${color[i+1]}`);
      sumVal += val;
    }
    sumVal /= 3;
    return sumVal;
  }



  // functions for small entrybox management
  function renderEntryBox() {

    if (props.c_state === 1) {
      return (
        <EntryBox data={props.data} callback={adjustData}
          tooltipdata={props.tooltipdesc}/>
      )
    }
    else {
      return (
        <div>
        </div>
      )
    }
  }

  function adjustData(new_data) {

    setData(new_data)
    // send new data to app
    callback(new_data)
  }



  // add in piechart graphic and entryboxes if necessary
  return (
    <div>
      <div className = "piehold">
        <svg
            className="pie-chart"
            width={dimensions.width + 2*dimensions.padding}
            height={dimensions.height + 2*dimensions.padding}
            ref={d3Container}
            overflow ="visible"
            
        />
        </div>
        {renderEntryBox()}

      
      </div>
    );
}


export default PieChartExpenditure;